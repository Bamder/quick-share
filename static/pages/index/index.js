...
} from "/static/utils/file-utils.js";
import {
  showMessage,
  showLoading,
  isValidPickupCode,
...
function updateUserStatusDisplay(user) {
  ...
  if (user) {
    ...
    userStatusEl.innerHTML = `
            <span class="greeting">${user.username} 您好</span>
        ...
        <div class="modal-body">
            <form id="registerForm">
                ...
                <div class="form-group">
                    ...
                    <input type="password" id="registerPassword" name="password" required placeholder="请输入密码">
                </div>
                <div class="form-group">
...
function initPasswordHashTest() {
  ...
  async function handleGenerateHash() {
    ...
    if (!password) {
      showMessage("请输入要测试的密码", "error");
      return;
    }

    try {
      showLoading(true, "正在生成哈希值...");
      ...
      // 显示结果
      plainPasswordEl.textContent = password;
      hashedPasswordEl.textContent = hashedPassword;
      passwordLengthEl.textContent = password.length;
      hashLengthEl.textContent = hashedPassword.length;
      hashTimeEl.textContent = duration.toFixed(2);
      ...
      showLoading(false);
      showMessage("哈希值生成成功", "success");
    } catch (error) {
      showLoading(false);
      showMessage("哈希值生成失败: " + error.message, "error");
  ...
  function handleClear() {
    ...
    plainPasswordEl.textContent = "";
    hashedPasswordEl.textContent = "";
    passwordLengthEl.textContent = "";
    hashLengthEl.textContent = "";
    hashTimeEl.textContent = "";

    // 清空控制台
    console.clear();
    console.log("密码哈希测试已清空");
  }

  // 绑定事件
  hashBtn.addEventListener("click", handleGenerateHash);
...
async function generatePickupCodeHandler() {
  ...
  try {
    ...
    const requestData = {
      ...
      size: Number.isInteger(CONFIG.localFile.size)
        ? CONFIG.localFile.size
        : Math.floor(CONFIG.localFile.size), // 确保是整数
    ...
    console.log("请求数据类型验证:", {
      size: {
        ...
        type: typeof requestData.size,
        isInteger: Number.isInteger(requestData.size),
      },
      limitCount: {
        value: requestData.limitCount,
        type: typeof requestData.limitCount,
        isInteger: Number.isInteger(requestData.limitCount),
      },
      expireHours: {
    ...
    // 计算文件哈希（用于文件去重和密钥缓存）
    try {
      const { calculateFileHash } = await import(
        "/static/utils/encryption-utils.js"
      );
      CONFIG.fileHash = await calculateFileHash(CONFIG.localFile);
      requestData.hash = CONFIG.fileHash;
      console.log(
        "文件哈希计算完成:",
        CONFIG.fileHash.substring(0, 16) + "..."
      );
    } catch (error) {
  ...
  try {
    ...
    senderService.init(`${CONFIG.API_BASE}/v1`, {
      ...
      onComplete: () => {
        ...
        // 显示完成标记
        const uploadCompleteBadge = document.getElementById(
          "uploadCompleteBadge"
        );
    ...
    await senderService.uploadFileViaRelay(
      CONFIG.pickupCode,
      CONFIG.localFile,
      CONFIG.fileHash,
      CONFIG.expireHours || 24
    );
  ...
  try {
    ...
    receiverService.init(`${CONFIG.API_BASE}/v1`, {
      onProgress: (progress, received, total, message) => {
        console.log(
          `下载进度: ${progress.toFixed(1)}% (${received}/${total})`,
          message || ""
        );
...
function updateStatusDisplayFromServer(data) {
  ...
  if (timeElement && data.expireAt) {
    ...
    try {
      ...
      if (
        !expireAtStr.includes("Z") &&
        !expireAtStr.match(/[+-]\d{2}:\d{2}$/)
      ) {
...
function startCountdown() {
  ...
  if (
    CONFIG_EXPIRE_TIME &&
    CONFIG_EXPIRE_TIME.getTime() <= new Date().getTime()
  ) {
    // 已过期，不创建定时器
    return;
  }
...
async function testPortConnectionHandler() {
  ...
  try {
    ...
    if (isConnected) {
      ...
      // 兼容性：如果页面是HTTPS，使用HTTPS；如果是HTTP，使用HTTP
